...
public class ServiceGenerator {
    ...
    private static autentication tipoActual = null;
    private static HttpLoggingInterceptor logging = new HttpLoggingInterceptor().setLevel(HttpLoggingInterceptor.Level.BODY);
    private static OkHttpClient.Builder httpClientBuilder = new OkHttpClient.Builder();

    //Sin autenticación
    public static <S> S createService(Class<S> serviceClass) {
        return createService(serviceClass, null, autentication.SIN_AUTENTICACION);
    }
    ...
    public static <S> S createService(Class<S> serviceClass, String username, String password) {
        if (!(username.isEmpty() || password.isEmpty())) {
            ...
            return createService(serviceClass, credentials, autentication.BASIC);
        }
        return createService(serviceClass, null, autentication.SIN_AUTENTICACION);
    }

    public static <S> S createService(Class<S> serviceClass, final String authtoken, final autentication tipo) {
        if (retrofit == null || tipoActual != tipo) {
            ...
            if (tipo == autentication.SIN_AUTENTICACION || tipo == autentication.BASIC) {
                httpClientBuilder.addInterceptor(new Interceptor() {
                    @Override
                    public Response intercept(Chain chain) throws IOException {
                        Request original = chain.request();
            ...
            if (authtoken != null) {
                httpClientBuilder.addInterceptor(new Interceptor() {
                    ...
                    public Response intercept(Chain chain) throws IOException {
                        Request original = chain.request();
                        String token = null;
                        if (tipo == autentication.JWT && !authtoken.startsWith("Bearer"))
